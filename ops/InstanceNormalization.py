...
# added
    try:
        ...
        beta = tf.reshape(B, (1,len(B),1))
        gamma = tf.reshape(scale, (1,len(scale),1))
        beta = tf.keras.layers.Resizing(len(beta),shape[2])(beta)
        gamma = tf.keras.layers.Resizing(len(gamma),shape[2])(gamma)
        beta = tf.transpose(beta,[2,0,1])
        gamma = tf.transpose(gamma,[2,0,1])
# upto
